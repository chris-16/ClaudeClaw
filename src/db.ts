...
function createSchema(database: Database.Database): void {
  database.exec(`
    ...
    -- App settings: key-value store for runtime-configurable settings (e.g. daily cost limit).
    ...
    -- Working memory: short summary of recent conversation context per chat+agent.
...
export function getAppSettingMeta(key: string): { value: string; updated_at: number } | undefined {
  return db.prepare('SELECT value, updated_at FROM app_settings WHERE key = ?').get(key) as { value: string; updated_at: number } | undefined;
}
...
export function getAgentCostBreakdown(days = 1): Array<{ agent_id: string; cost: number; turns: number }> {
  const cutoff = Math.floor(Date.now() / 1000) - days * 86400;
  return db.prepare(
    `SELECT agent_id, SUM(cost_usd) as cost, COUNT(*) as turns
     FROM token_usage WHERE created_at > ?
     GROUP BY agent_id ORDER BY cost DESC`,
  ).all(cutoff) as Array<{ agent_id: string; cost: number; turns: number }>;
}

export function getTopCostlyQueries(limit = 10, days = 7): Array<{ session_id: string; cost_usd: number; output_tokens: number; created_at: number }> {
  const cutoff = Math.floor(Date.now() / 1000) - days * 86400;
  return db.prepare(
    `SELECT session_id, cost_usd, output_tokens, created_at
     FROM token_usage WHERE created_at > ? AND cost_usd > 0
     ORDER BY cost_usd DESC LIMIT ?`,
  ).all(cutoff, limit) as Array<{ session_id: string; cost_usd: number; output_tokens: number; created_at: number }>;
}

// ── Activity Log ──────────────────────────────────────────────────

export function logActivity(type: string, description: string, agentId = 'main', metadata?: string): void {
  const now = Math.floor(Date.now() / 1000);
  db.prepare(
    'INSERT INTO activity_log (type, description, agent_id, metadata, created_at) VALUES (?, ?, ?, ?, ?)',
  ).run(type, description, agentId, metadata ?? null, now);
}

export function getActivityLog(limit = 50, offset = 0, type?: string): Array<{ id: number; type: string; description: string; agent_id: string; metadata: string | null; created_at: number }> {
  if (type) {
    return db.prepare(
      'SELECT * FROM activity_log WHERE type = ? ORDER BY created_at DESC LIMIT ? OFFSET ?',
    ).all(type, limit, offset) as Array<{ id: number; type: string; description: string; agent_id: string; metadata: string | null; created_at: number }>;
  }
  return db.prepare(
    'SELECT * FROM activity_log ORDER BY created_at DESC LIMIT ? OFFSET ?',
  ).all(limit, offset) as Array<{ id: number; type: string; description: string; agent_id: string; metadata: string | null; created_at: number }>;
}

// ── Working Memory ──────────────────────────────────────────────────

export function getWorkingMemory(chatId: string, agentId = 'main'): string {
  const row = db.prepare(
    'SELECT summary FROM working_memory WHERE chat_id = ? AND agent_id = ?',
  ).get(chatId, agentId) as { summary: string } | undefined;
  return row?.summary ?? '';
}

export function setWorkingMemory(chatId: string, summary: string, agentId = 'main'): void {
  const now = Math.floor(Date.now() / 1000);
  db.prepare(
    'INSERT OR REPLACE INTO working_memory (chat_id, agent_id, summary, updated_at) VALUES (?, ?, ?, ?)',
