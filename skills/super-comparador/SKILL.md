...
# Super Comparador — Jumbo vs Lider vs Tottus
...
**Herramienta de browser**: Claude in Chrome (`navigate`, `find`, `read_page`,
`get_page_text`). NO usar Playwright.
...
### Fase 2 — Scraping de precios (los 3 sitios)

Buscar cada producto en los 3 sitios **secuencialmente** (un sitio completo antes
de pasar al siguiente). Por cada producto en cada sitio:
...
**Fallback si la URL de búsqueda no muestra resultados:**
Usar el buscador del sitio directamente: click en el input de búsqueda, escribir el producto, y presionar Enter. Esperar 3-4 segundos a que carguen los resultados dinámicos.

**Nota sobre Jumbo:** La URL `?q=` muestra página 404. SIEMPRE usar `?ft=`.
**Nota sobre Lider:** El dominio principal redirige a `super.lider.cl`. Ir directo ahí. Los resultados de búsqueda se cargan debajo del fold — hacer scroll down para verlos.

**Por cada resultado capturar:**
...
| Sitio lento / timeout | Reintentar 1 vez. Si falla: continuar con N/D para ese sitio |
| Producto no encontrado | ❌ N/D, excluir del total de ese super |
| Precio no visible | Marcar como "precio no disponible", no asumir |
...
- **WebFetch NO funciona** para estos sitios — los productos se cargan dinámicamente con JavaScript. SIEMPRE usar Chrome.
...
### Sobre los sitios
- Los 3 sitios cargan productos via JavaScript/SPA — WebFetch solo obtiene el shell HTML sin datos. **Obligatorio usar Chrome**.
- Jumbo requiere login para ver precios de despacho (Chris ya tiene sesión activa con Jumbo Prime).
...
- Los 3 sitios muestran un cookie banner. Cerrar con X (no "Aceptar") cuando sea posible.
